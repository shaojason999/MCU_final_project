...
# 20 "./Configuration_header_file.h" 2
...
# 89 "./Configuration_header_file.h"
#pragma config FCMEN = OFF
...
#pragma config EBTRB = OFF
# 7 "PIC_Interface_with_XBee.c" 2

# 1 "./LCD_16x2_Header_File.h" 1
# 18 "./LCD_16x2_Header_File.h"
void LCD_Init();
...
# 9 "PIC_Interface_with_XBee.c" 2
# 35 "PIC_Interface_with_XBee.c"
uint8_t ReceiveBuffer[100];
int8_t DigitalData[16];
int16_t AnalogData[8];
uint16_t BufferPointer = 0,
   LastByteOfFrame = 0;
uint32_t Command_Value = 0;

_Bool _IsContainsDigital = 0,
  _IsContainsAnalog = 0;
...
void sample()
{
 ...
 {
  case (0x92):
   if(Is_Data_Received() == 0 || Is_Checksum_Correct() == 0) break;
...
{
 MSdelay(200);
 for (uint16_t count = 0; Is_Data_Received() == 0; count++)
 {
  if(count>15000)
  {
   return 0;
...
void Remote_AT_Command(uint32_t Long_Address_MSB, uint32_t Long_Address_LSB, uint16_t Short_Address, const char* ATCommand, _Bool action)
{
 ...
 Checksum = Checksum + (Short_Address >> 8) + Short_Address + 0x02 + ATCommand[0] + ATCommand[1];
 if (action == 1)
...
void AT_Command(const char* ATCommand, _Bool action)
{
 uint16_t Length,Checksum;
 if (action == 1)
 {
 ...
 Checksum = 0x08 + 0x01 + ATCommand[0] + ATCommand[1];
 if (action == 1)
 Checksum = Checksum + (Command_Value >> 24) + (Command_Value >> 16) + (Command_Value >> 8) + Command_Value ;
 Checksum = 0xFF - Checksum;
...
void Write_AT_Command(char* ATCommand, uint32_t _CommandValue)
{
 ...
 AT_Command(ATCommand, 1);
}

void Read_AT_Command(char* ATCommand)
{
 AT_Command(ATCommand, 0);
}
...
{
 Command_Value = _CommandValue;
 Remote_AT_Command(Long_Address_MSB, Long_Address_LSB, Short_Address, ATCommand, 1);
}
...
int main(void)
{
 ...
 while (1)
 {
  ...
  if (_IsContainsAnalog)
  {
   ...
   if(AnalogData[0] <=1523 && AnalogData[0]>=900 && AnalogData[2] <=1523 && AnalogData[2]>=900){
                LATDbits.LATD1=0;
                LATCbits.LATC3=0;
            }
   else if(AnalogData[0] <=650 && AnalogData[0]>=350){
                LATDbits.LATD1=1;
                LATCbits.LATC3=0;
            }
   else if(AnalogData[0] <=200 && AnalogData[0]>=0 && RC0!=0){
                LATCbits.LATC3=1;
                LATDbits.LATD1=0;
            }


            if(AnalogData[2] <=1523 && AnalogData[2]>=900){
                ;
            }
   else if(AnalogData[2] <=650 && AnalogData[2]>=350){
                ;
            }
   else if(AnalogData[2] <=200 && AnalogData[2]>=0){
                LATDbits.LATD1=1;
                LATCbits.LATC3=0;
            }
  }
